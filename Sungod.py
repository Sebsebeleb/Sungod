...
pie_jokes = (
    ...
    ("Why did the wounded ask for painkillers?", "It was too pienful for him."),
    ...
    ("Why do fish not like basketball?", "Because they are afraid of the net!"),
...
class Ninjafy(basecmd):
   ...
   "x":"na","y":"fu","z":"zi"," ":" ", "?":"??"}
    ...
    def do(self,args,connection,event):
        ...
        say("".join([self.mapping[i] for i in name]).capitalize(),event.target)
...
class Pynify(basecmd):
    ...
    def do(self,args,connection,event):
        try:
            ...
            connection.privmsg(event.target,"Here is your short url, smallson: " + str(result))
    ...
    def do(self, args, connection, event):
        try:
            ...
            if u in stats["users"].keys():
                ...
                except AttributeError: print "User did not have this stat before."
                except KeyError: connection.privmsg(event.target, "Sorry, user '" + u + "' not found.")
...
class Quote(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if args == "list":
            ...
            connection.privmsg(event.target,'"'+ '", "'.join(self.quotes.keys())+'"')
        ...
        if quote:
            connection.privmsg(event.target, "Let me tell you a story, youngster. It was a sunny day, when this was said...")
            for x in quote:
...
class RandomQuote(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        try:
            ...
            quotes = [stats.users[u].lines[random.randrange(len(stats.users[u].lines))] for x in range(n)]
        except IndexError:
...
class TopSpeakers(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if not len(args):
            ...
            try: n = int(args[0])
            except IndexError: connection.privmsg(event.target, "Bad arg.")


        connection.privmsg(event.target, ("Top speakers of "+ event.target).center(45,"*"))
        for rank, u in enumerate([i[1] for i in reversed(sorted(stats.users.items(), key = lambda t: t[1].lines_spoke))][:n]):
            connection.privmsg(event.target,((("   %s. is %s with %s lines"%((rank+1,u.name,u.lines_spoke))).ljust(35, " ")).center(45, "*")))
...
class DisplayHelp(basecmd):
    ...
    def do(self, args, connection, event):
        connection.privmsg(event.target, "Possible commands: " + str(list(c for c in reversed(sorted(pubcommands.values(), key = lambda t: t.powerreq)))))
...
class DFmoral(basecmd):
    ...
    def do(self, args, connection, event):
        site = urllib.urlopen("http://df.magmawiki.com/index.php/Main_Page").read()
        rex = re.compile(r'<div style="float: right; margin-left: 1em"><div style=.+>(.+)</div></div>')
#        l = True
#        while l != "":
...
class Random(basecmd):
    ...
    _support = ["Midas (semi)", "Myrmidon", "Witch Slayer", "Pyromancer",
        ...
        "Ophelia"]
    _w0bstyle = ["Enigma","Rikimaru","Roshan", "Slather", "Chokochick", "Batni",
        "Shrut", "Codex 5", "SotM NH", "Slothni", "Codex Queen", "Fundromeda", "Pesto",
        "Etter", "Titstandul", "HERREX!!!!","Funarch"]
    _pusher = ["Tempest", "Keeper of the Forest", "Pollywog", "Torturer", "Balphagore",
        "Hellbringer (semi)", "Demented shaman (semi)", "Rhapsody (semi)", "Soul reaper",
        "Parasite (semi)", "Legionaire (cskip)", "Zeph (cskip)"]
    _push = _pusher
    _nuker = ["Pyromancer", "Torturer", "Thunderbringer", "Behemoth", "Wretched hag"]
    _ganker = ["Pyromancer", "Torturer", "Thunderbringer", "Behemoth", "Wretched hag",
    ...
    _hard_carry = ["Madman", "Chronos", "Swiftblade","Moon Queen","Wild Soul","Puppet Master",
        "Vindicator","Soulstealer","Night Houn","Gemini","Dampeer","Soul Reaper"]
    _kongor = ["Wild Soul", "Valkyrie", "Gauntlet", "Slither", "Pharaoh", "Plague Rider",
        "War Beast", "Demented Shaman"]
    _healer = ["Soul reaper", "Nymphora", "Demented shaman", "Jereziah", "Monarch",
    ...
    validargs = ["heroes","nuker","w0bstyle","push/pusher", "heal(er)","hard_carry","kongor"    ]
    ...
    def do(self, args, connection, event):
        ...
        try:
            ...
            print result
            if times > 1:
                connection.privmsg(event.target, "Sungod chooses... " + ", ".join(result[:-1]) + " and " + result[-1] + "!")
            else:
                connection.privmsg(event.target, "Sungod chooses... " + result[0])
...
class Timer(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if who == speaker:
            ...
            s = who
        s = s + " will be reminded at " + str(trigger_date.strftime("%d.%m.%Y %H:%M:%S"))
        ...
        irc.execute_delayed(delay, self._remind, (who, event.target, message, connection))
...
class RPS(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        for u in self.players:
            ...
            self.answers[u] = a
            print a
            connection.notice(u,"Choose wisely!; Rock: %s,Paper: %s, Scissor: %s"%tuple(a))
    ...
    def on_talk(self, connection, event):
        ...
        if speaker in self.players_wait and msg in ["1","2","3"]:
            ...
            self.answers[speaker] = ["Rock","Paper","Scissors"][self.answers[speaker].index(int(msg)-1)]
    ...
    def finish(self, connection, target):
        ...
        if p1a == "Rock" and p2a == "Paper":
            ...
            a = False
        if a:
            connection.privmsg(target, "And the winner is... " + winner + "(" + a + ")")
        else:
        ...
        if args[0] in self.commands:
            ...
            if len(args) > 1:
                ...
                if not failed:
                    ...
                    connection.privmsg(event.target, "Spelling error, (" + " ".join(s) + ")")
            else:
                a = args[-1]
                s = (d.check(a) and "Found " or "Not found ") + "related: " + ", ".join(d.suggest(a))
                connection.privmsg(event.target, s)
...
class Charges(basecmd):
    ...
    trigger = "charges"
    def do(self, args, connection, event):
        if not (stats["users"].has_key("w0bni")):
            connection.privmsg(event.target, "w0bni is clouded before my eyes!")
            return
        w = stats["users"]["w0bni"]
        if speaker != "Frets" and args[0] not in ["get","check","remaining"] and args[0] in ["add","remove"]:
            if speaker == "w0bni":
                connection.privmsg(event.target, "GOOD TRY MADS. Sin increased by 1")
            ...
            try:
                ...
                if cmd == "remove":
                    w.charges -= 1
                    if w.charges < 0:
                        connection.privmsg(event.target, "w0bni did not have any charges at the moment.")
                        w.charges = 0
                elif cmd == "add":
                    w.charges += 1
                    if w.charges > 2:
                        w.charges = 2
                        connection.privmsg(event.target, "w0bni already had max amount of charges")
            except AttributeError as e:
                    print e
                    connection.privmsg(event.target, "w0bni did not have charges leveled up, leveling it up for him...")
                    w.charges = 1
        if args[0] in ["get","check","remaining"]:
            connection.privmsg(event.target, "w0bni has " + str(w.charges) +" left at the moment.")
...
class Dictionary(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if not d:
            import enchant
            d = enchant.Dict(stats["spell_dict"])
            s = "'" + " ".join(args) + "' was not found, maybe you meant; " + ", ".join(d.suggest(" ".join(args))) + "?"
            connection.privmsg(event.target,s)
        else:
            for i in d:
...
class wob(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        response = ("I am so sorry, " if wobStatus[0] == "0" else "Indeed, ") + wobStatus[1]
        connection.privmsg(event.target, response)
...
class Top(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if len(args) == 1:
            ...
            if len(args) == 3:
                n = int(args[1] != "reversed" or args[1] == "-r" and args[1] or args[2])
            reverse = False
        else:
            try:
                n = int(args[1])
        ...
        if not hasattr(stats["users"].values()[0],stat):
            connection.privmsg(event.target, "Sorry, '" +stat+"' is not known to me")
            return
        ...
        if reverse:
            ...
            l = [(len(stats["users"].values()) - r+1,i[0],i[1]) for r,i in enumerate(l,len(stats["users"]))]
        for rank, user, value in l:
            connection.privmsg(event.target,(((("*"*5+" "*5+("%s. is %s with %s %s"%(rank,user,value,stat)))).ljust(50, " ")).ljust(55, "*")))
...
class Time(basecmd):
    ...
    def do(self, args, connection, event):
        ...
        if index == 4:
            index = 0
        answer = ["It is HON O' CLOCK!", "It is quarter past HON TIME!", "It is half past HON TIME!", "It is quarter to HON TIME!"][index]
...
def handlePubMessage(connection,event):
    ...
    speaker = event.source.split('!') [0]
    message = event.arguments[0]
    if stats["users"].has_key(speaker):
        stats["users"][speaker].last_line = message
    time_stamp = time.strftime("[%H:%M:%S]")

    if "http://localhost" in message and "bni" in speaker:
        connection.privmsg(event.target,"He meant "+message.replace("localhost","84.215.30.88"))
    ...
    if not stats["users"].has_key(speaker):
    ...
    try:
        if spot:
            ...
            if 'find that' in str(sup):
                connection.privmsg(event.target,"That is _so_ not a working spotify URI.")
            else:
                artist = artre.findall(str(sup))
                song = songre.findall(str(sup))
                tags = song[0].split(" ")
                ulink = 'https://gdata.youtube.com/feeds/api/videos?orderby=viewCount&q='+"+".join(tags)
                getTube = urllib.urlopen(ulink).read()
                countRes = getTube.split("totalResults>")[1].split("</")[0]
                if countRes == "0":
                    tubeOut = ""
                #elif artist in getTube: helt serr, få inn denne - det blir kung. Statistikk basert på 2 testsøk viser det!
                else:
                    tubeOut = " (http://youtu.be/"+getTube.split("watch?v=")[1].split("&amp")[0]+")"
    ...
    if "dun " in message.lower():
        connection.privmsg(event.target, "http://www.youtube.com/watch?v=bW7Op86ox9g")

    elif message == "!":
        connection.privmsg(event.target, "http://www.youtube.com/watch?v=2P5qbcRAXVk")

    elif "ba dum" in message.lower():
        connection.privmsg(event.target, "http://www.youtube.com/watch?v=bcYppAs6ZdI")

    elif "excellent" in message.lower():
        connection.privmsg(event.target, "http://bbg.terminator.net/excellent.mp3")
        ...
        connection.privmsg(event.target, "http://bbg.terminator.net/smashing.mp3")

    elif "embarrasing" in message.lower() and speaker.lower() == "blam":
        connection.privmsg(event.target, "http://bbg.terminator.net/embarrasing.mp3")

    m = re.search(r"r/\w+",message,re.IGNORECASE)
    if m:
        connection.privmsg(event.target,"http://www.reddit.com/"+m.group().strip(" "))
    ...
    if event.arguments[0].lower().startswith(connection_info["prefix"].lower()):
        ...
        if cmd[0] == "/":
            ...
            if int(stats["users"][speaker].power) <= int(pubcommands[cmd].powerreq) or speaker.lower() == "sebsebeleb" or speaker.lower() == "w0bni":
                ...
                print "User tried to perform " +  cmd +", but his/her power isnt high enough (" + str(stats["users"][speaker].power) + " vs " + str(pubcommands[cmd].powerreq) + ")"
                connection.privmsg(event.target,"Sorry, not enough power. (" + str(stats["users"][speaker].power) + " vs " + str(pubcommands[cmd].powerreq) + ")")
        ...
        #TO-BE-DONE-LATER: ^ Needs a better implementation. "What is happening, Sungod?" would send it as "What is happening," rather than the inteded "What is happening?"
...
def sungod_says(msg, connection, event, speaker):
    ...
    if raw_str == "what is a man?":
        ...
        sendNote = urllib.urlopen("http://bbg.terminator.net/desknotes/newnote.php?to="+str(to)+"&from="+str(bro)+"&msg="+str(msg))
        resp = BeautifulSoup(sendNote)
        if "Error" in str(resp):
            answer = "Sorry, "+str(speaker)+", I could not note that. The bokk might be broken."
        ...
        if t:
            ...
            if person:
                ...
                answers = [" is a man of great reknown. The great warm book tells only good of him.", " is a blasphemous nekro", "... How I hate him... Please, don't speak of him",
                ...
                answer = random.choice(["I am not sure","The bokk does not speak of him"])
        else:
        ...
        answers = ["Because I say so.", "It is written in the great warm bokk, therefore.", "It is undeniable", "for I made it so","", "Are you sure it is so? So thats what happenend that night..."]
        methods = 8
        ...
        if method == 1 or method == 2:
            ...
            if u == speaker.capitalize():
                u = "yourself"
            prefix = random.choice(["Maybe you should ask","Ask", "Do not waste my time, ask", "It would be wise to ask", "Warm book says you should ask"])
            suffix = random.choice(["that question.", "yourself", "before I go mad and send you to nekromans or devell!"])
            answer = prefix + " " + u + " " + suffix
        elif method == 4:
            prefix = random.choice(["I hear", "I believe", "Check with", "It could be that", "According to the warm bokk,", "The fish say"])
            suffix = random.choice(["has great knowledge regarding that","is more than able to answer you that", "is in possesion of the answer for that", "should have the answer"])
            u = random.choice(stats["users"].keys()).capitalize()
        ...
        if who:
            ...
            if t == 1:
                ...
                if method == 1:
                    prefix = random.choice(["Last I checked, he was ", "I believe you will find him ", "Definitivly ",
                                            "According to the light, he is "])
                    suffix = random.choice(["in the shapell", "outside", "reading in the great bokk, outside the shapell",
                    ...
                    while u == speaker.capitalize() and u != who.capitalize():
                        u = random.choice(stats["users"].keys())
                    answer = "I believe " + u + " is softing with " + who + ", those scandalous dimmers"
            else:
                answer = "God knows."
        else:
            answer = random.choice(["In the shapell", "With the bats", "Where the bats never leave",
                "Somewhere over the rainbow", "Not here atleast."])


    elif "when" == raw_str.lower().split(" ")[0]:
        answers = ["I would say about...", "Roughly", "As prophesied by the bokk,"]
        methods = 4
        method = random.randint(1,methods)
        if method == 1 or method == 2:
            prefix = random.choice(answers)
            answer = prefix + " " + str(random.randint(2,59)) + random.choice([" minutes"," seconds"])
        elif method == 3:
            answer = "You must wait " + random.choice(["one","two","three",
                                                        "eight"]) + " " + random.choice(["days","hours","sunsets", "years"])
        elif method == 4:
            t = random.choice(["The next time the sun sets", "When the bats cry", "When w0bni no longer leaks",
        ...
        if re.search("should .* do",raw_str.lower()):
            if "doing" in raw_str.lower():
                answers = ["Praying", "Bracing up", "Looking for the bat cave to hide in it",
                            "Running away", "Hiding", "Speaking with baptist","Giving a gift to Baptist",
                            "Finding fish", "Fishing", "Sleeping with fishes","Teaching"]
            else:
                answers = ["Pray", "Brace yourself", "Look for the bat cave and hide in it",
            ...
            if method == 1:
                answer = random.choice(["Many things can happen, if you pray you will be fine.",
...
#       answers =
        ...
        answers = ["abseloteli","deliberator", "no shancè", "as clair as my lihgt",
        ...
        if method == 1 or method == 2 or method == 11:
            ...
            answer = "As said in the great warm bokk: " + '"' + midfix + '"' + subfix
#        elif method == 8:
#            answer = "Did you know I am very far away from you?"
        elif method == 8 or method == 5:
            answer = "Is '" + midfix + "' the answer you seek, " + random.choice(["by chance?", "perhaps?", "or is my wisdom unfilling?"])

    elif raw_str.endswith("!"):
        answers = ["Don't you dare!", "Who do you think you are, trying to command me like that?",
...
def court(speaker,message,chn):
    if speaker.lower() in [s.lower() for s in stats["users"].keys()]:
        ...
        if math.sqrt(speaker_faith) > total_faith * faithful_cap:
            say("Oh no! How could someone as pure as "+speaker+" have commited such mischief?",chn)
            ...
            if random.randint(1,2) == 1:
                influencer = random.choice([u.name for u in stats["users"].values() if math.sqrt(u.faith) < total_faith * evil_cap])
                if influencer:
                    s = s + " Maybe it was the act of the foul " + influencer + "."
            say(s,chn)
        else:
...
def on_disconnected(socket):
    ...
    server.connect ( connection_info["network"], connection_info["port"], connection_info["nick"], ircname = connection_info["name"] )
...
def load_commands(cmd):
    ...
    commands = None
    while 1:
        try: cmodule = __import__(cmd)
        except TypeError: print "Failed importing " + str(cmd) + ", possibly not a module."
        else:
            commands = cmodule.commands
            break

        try: commands = list(cmd)
        except TypeError: print "Failed making a list of " + str(cmd) + ", possibly not a list of commands."
        else: break#dancing is cool. breakdancing.

    for c in commands:
        if c.type == "pub":
            global pubcommands
            if pubcommands.has_key(c.trigger):
...
def tell_pie_jokes():
    ...
    for chan in config.get("startup","channels").split():
        ...
        irc.execute_delayed(11,server.privmsg, (chan, "http://www.youtube.com/watch?v=_Rav9ijyyZk"))
